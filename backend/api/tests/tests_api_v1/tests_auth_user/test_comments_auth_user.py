...
class TestAuthUserComments(APITestCase):
  ...
  def test_auth_user_can_list_all_comments(self):
  ...
  def test_auth_user_can_create_comments(self):
    data = {
  ...
  def test_auth_user_can_list_comments_by_all_fields(self):
  ...
  def test_auth_user_can_retrieve_comment(self):
  ...
  def test_auth_user_cannot_retrieve_comment_that_does_not_exist(self):
    response = self.client.get(path=reverse('comment', args=[999]))
    self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

  
  def test_auth_user_cannot_put_comment(self):
  ...
  def test_auth_user_cannot_patch_comment(self):
  ...
  def test_auth_user_cannot_delete_comment(self):
    response = self.client.delete(path=reverse('comment', args=[self.comments_1.id]))
    self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

  
  def test_auth_user_can_update_his_comment(self):
    ...
    response = self.client.put(path=reverse('comment', args=[comment.id]), data=data)
    self.assertEqual(response.status_code, status.HTTP_200_OK)


  def test_auth_user_can_patch_his_comment(self):
    comment = CommentFactory(user=self.user)
    data = {
      'lesson': comment.lesson.id,
      'stars': 5,
    ...
    response = self.client.patch(path=reverse('comment', args=[comment.id]), data=data)
    self.assertEqual(response.status_code, status.HTTP_200_OK)

  def test_auth_user_can_delete_his_comment(self):
    comment = CommentFactory(user=self.user)
    response = self.client.delete(path=reverse('comment', args=[comment.id]))
